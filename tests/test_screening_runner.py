...
def test_plan_only_cli_reads_preflight_without_writing_status(tmp_path: Path) -> None:
    root = tmp_path / "project"
    (root / "data/manifests").mkdir(parents=True)
    pool_path = root / "data/manifests/screening_pool.tsv"
    ...
    config = root / "screening_pool.yaml"
    config.write_text(
        "\n".join(
            [
                f"project_root: {root}",
                "batch:",
                "  local_permutations: 10",
                "  pair_permutations: 10",
    ...
    result = subprocess.run(
        [
            sys.executable,
            "scripts/14_run_screening_pool.py",
            "--config",
            str(config),
            "--pool",
            str(pool_path),
    ...
    assert "unsupported_afdb_fragment" in result.stdout
...
def test_repeated_cached_resume_does_not_inflate_attempt_history(
    tmp_path: Path,
) -> None:
    root = tmp_path / "project"
    candidate = _candidate(
    ...
    _write_valid_robust(root, candidate, with_segments=False)
    (root / "data/manifests").mkdir(parents=True, exist_ok=True)
    pool_path = root / "data/manifests/screening_pool.tsv"
    preflight_path = root / "data/manifests/screening_pool_preflight.tsv"
    pd.DataFrame([candidate]).to_csv(pool_path, sep="\t", index=False)
    pd.DataFrame(
        [
            {
                "screening_index": 24,
    ...
    first_lines = status_path.read_text(encoding="utf-8").splitlines()
    ...
    second_lines = status_path.read_text(encoding="utf-8").splitlines()
