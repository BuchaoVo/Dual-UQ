...
def read_single_fasta(path: Path) -> str:
    ...
    if header_count != 1 or len(sequences) != 1:
        raise ValueError(
            f"{path}: expected one FASTA record, found "
            f"{header_count} headers and {len(sequences)} sequences."
        )
    return sequences[0]


def locate_npz(output_dir: Path) -> Path:
    candidates = sorted(
        path
    ...
    # ProteinMPNN score_only + path_to_fasta intentionally writes two files:
    #   <structure>_pdb.npz      for the sequence encoded by the structure
    #   <structure>_fasta_1.npz  for the supplied candidate FASTA
    # Cross-scoring must use the FASTA-derived record.
    fasta_candidates = [
    ...
    if len(fasta_candidates) == 0:
        raise ValueError(
            f"{output_dir}: no *_fasta_1.npz score-only output found; "
            f"all NPZ files={len(candidates)}: "
            f"{[str(path) for path in candidates]}"
        )
    raise ValueError(
        f"{output_dir}: multiple *_fasta_1.npz outputs found: "
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Validate one completed ProteinMPNN score-only job."
    )
    ...
    try:
        ...
        npz_path = locate_npz(args.output_dir)
        ...
        if not np.array_equal(observed_s, expected_s):
            raise ValueError(
                f"{npz_path}: scored sequence does not match candidate FASTA."
            )
