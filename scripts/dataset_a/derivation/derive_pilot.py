...
def _validated_preflight(paths: ProjectPaths) -> tuple[dict[str, Any], dict[str, Any]]:
    report_paths = {
        "inventory": paths.reports_root / "dataset_a_scale/candidate_inventory_v1.json",
        "batch1": paths.reports_root / "dataset_a_scale/batch1_acquisition_plan_v1.json",
        "state": paths.reports_root
        / "dataset_a_scale/batch1_acquisition_current_state_v2.json",
    }
...
def _successful_assets(
    ...
    acq1 = _load_json(
        paths.reports_root / "dataset_a_scale/batch1_acquisition_run_v1.json"
    )
    rows = [
        dict(row)
...
def _candidate_context(
    ...
    *,
    ...
    exact = extract_canonical_sequence(
...
def _run_pilot(paths: ProjectPaths) -> dict[str, Any]:
    ...
    plan = pd.read_csv(
        paths.reports_root / "dataset_a_scale/batch1_plan_v1.tsv", sep="\t"
    ).to_dict("records")
    acquisition_rows = attach_batch1_ranks(acquisition_rows, plan)
    resolution_report = _load_json(
        paths.reports_root
        / "dataset_a_scale/afdb_metadata_record_resolution_v1.json"
    )
    resolution = {
...
def main() -> int:
    ...
    output_dir = args.output_dir or paths.reports_root / "dataset_a_scale"
