...
INVENTORY_PATH = Path("reports/dataset_a_scale/candidate_inventory_v1.json")
BATCH1_PATH = Path("reports/dataset_a_scale/batch1_plan_v1.tsv")
...
def write_outputs(payload: Mapping[str, Any], output_dir: Path) -> None:
    ...
    (output_dir / "batch1_acquisition_plan_v1.json").write_text(
        json.dumps(payload, indent=2, sort_keys=True, ensure_ascii=False) + "\n",
        encoding="utf-8",
    )
    _write_tsv(output_dir / "batch1_acquisition_plan_v1.tsv", list(payload["records"]))
    (output_dir / "batch1_acquisition_plan_v1.md").write_text(
