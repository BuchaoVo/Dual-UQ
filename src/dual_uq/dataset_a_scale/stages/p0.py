...
from dual_uq.dataset_a_scale.lifecycle import evaluate_resume
from dual_uq.dataset_a_scale.pae import (
    AFDBFragment,
    PAEMappingError,
    load_pae_json,
    require_fragment_coverage,
)
from dual_uq.dataset_a_scale.schema import (
...
from dual_uq.dataset_a_scale.structures import ResidueKey, ResidueProvenance
...
def _atomic_write_new(path: Path, payload: bytes) -> None:
    ...
    try:
        ...
        try:
            ...
            raise P0ValidationError(
                "immutable_output_exists", f"Refusing to overwrite immutable P0 output: {path}"
            ) from exc
    finally:
