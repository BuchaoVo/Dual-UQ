...
OUTPUT_DIR = Path("artifacts/dataset/reports/census")
EXPECTED_TOTAL = 220
...
def _round1_evidence(project_root: Path) -> dict[str, dict[str, Any]]:
    payload = _read_json(
        project_root / "artifacts/dataset/reports/census/round1_report.json"
    )
...
def write_outputs(payload: Mapping[str, Any], output_dir: Path) -> None:
    ...
    atomic_write_text(markdown_path, _markdown(payload))
