...
LOG_FILE="${V2_ROOT}/logs/v2c_step4_step5_recovery_$(date +%Y%m%d_%H%M%S).log"
...
echo "Recovered score jobs: ${#JOB_LINES[@]}"
...
echo "Step 4 recovery: validate or run all 18 score jobs"
...
for LINE in "${JOB_LINES[@]}"; do
  ...
  if [[ -n "${EXTRA:-}" ]]; then
    echo "Unexpected extra TSV field in row: ${LINE}"
    exit 2
  fi
  if [[ ! -f "${FASTA_PATH}" ]]; then
    echo "Candidate FASTA not found: ${FASTA_PATH}"
    exit 2
  fi
...
echo
echo "Validate complete score-job count"
...
if [[ "${COMPLETE}" -ne 18 ]]; then
  echo "Not all score jobs are complete; Step 5 will not run."
  exit 1
fi
...
echo
echo "V2C Step 4/5 recovery completed."
echo "Review:"
